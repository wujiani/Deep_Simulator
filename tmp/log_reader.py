...
class LogReader(object):
    ...
    def reorder_xes(self, temp_data):
        ...
        if self.one_timestamp:
            self.column_names['Complete Timestamp'] = 'start_timestamp'
            temp_data = temp_data[temp_data.event_type == 'complete']
            ordered_event_log = temp_data.rename(
                columns={'timestamp': 'start_timestamp'})
            ordered_event_log = ordered_event_log.drop(columns='event_type')
            # ordered_event_log = ordered_event_log[["caseid", "task", "user", "start_timestamp"]]
            # ordered_event_log['end_timestamp'] = ordered_event_log.groupby('caseid')['start_timestamp'].shift(-1)
        ...
        for case, group in pd.DataFrame(self.data).groupby('caseid'):
            ...
            if self.one_timestamp:
                end_start_times[(case, 'End')] = (
                        group.start_timestamp.max() + timedelta(microseconds=1))
            else:
                end_start_times[(case, 'End')] = (
